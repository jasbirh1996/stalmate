...
import com.slatmate.user.networking.RestClient
import com.slatmate.user.utility.ValidationHelper
...
//        binding.etDOB.text = SimpleDateFormat("dd.MM.yyyy").format(System.currentTimeMillis())
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        val view =  inflater.inflate(com.slatmate.user.R.layout.fragmentsignup, container, false)
...
//        binding.etDOB.text = SimpleDateFormat("dd.MM.yyyy").format(System.currentTimeMillis())
    ...
    fun isValid():Boolean{
       ...
       if (ValidationHelper.isNull(binding.etName.text.toString())){
           ...
           makeToast(getString(com.slatmate.user.R.string.first_name_toast))
           return false
       }else
           if (ValidationHelper.isNull(binding.etLastName.text.toString())){
           makeToast(getString(com.slatmate.user.R.string.last_name_toast))
       }else
           if (!isValidEmail(binding.etEmail.text.toString())){
            makeToast(getString(com.slatmate.user.R.string.email_error_toast))
            return false;
        }else
            if(!binding.rdmale.isChecked && !binding.rdFamel.isChecked && !binding.rdOthers.isChecked){
            makeToast(getString(com.slatmate.user.R.string.select_gendar_error))
               return false
            }else
                if(ValidationHelper.isValidPassword(binding.etPassword.text.toString())){
                makeToast(getString(com.slatmate.user.R.string.password_error_toast))
                return false
            }else
                if (!binding.tmcheckbox.isChecked){
                makeToast(getString(com.slatmate.user.R.string.accept_tnc))
            }
        return true
    }
    ...
    private fun toolbarSetUp() {
        ...
        binding.toolbar.toolBarCenterText.text =  getString(com.slatmate.user.R.string.sign_up)
        binding.toolbar.toolBarCenterText.visibility = View.VISIBLE
